from microbit import *

initRegisterArray = [
    
]


class GESTURE(object):
    """基本描述

    PAJ7620U2, 手势传感器

    """

    def __init__(self):
        i2c.init()
        self.__initmodule()

    def __i2cwrite_gesture(self, addr, reg):
        i2c.write(0x73, bytearray([addr, reg]))

    def __i2cread_gesture(self, reg):
        i2c.write(0x73, bytearray([reg]))
        t = i2c.read(0x73, 1)
        return t[0]

    def __selectbank(self, bank):
        if bank == 0:
            self.__i2cwrite_gesture(0xEF, 0)
        elif bank == 1:
            self.__i2cwrite_gesture(0xEF, 1)

    def __initmodule(self):
        self.__selectbank(0)
        temp = self.__i2cread_gesture(0)
        if temp == 0x20:
            for i in range(0, 438, 2):
                self.__i2cwrite_gesture(initRegisterArray[i], initRegisterArray[i+1])
        self.__selectbank(0)

    def get_gesture(self):
        """

        读取当前手势值

        Returns:
            读取当前手势值
        """
        __tmp = self.__i2cread_gesture(0x43)

        return __tmp


if __name__ == '__main__':
    gesture = GESTURE()
    while True:
        print("GESTURE: ", gesture.get_gesture())
        sleep(5)
